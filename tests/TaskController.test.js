...
describe("insert", () => {
  ...
  it('creating a task', async () => {
    ...
    //ERROR
    const res_create_task_error = await request.post('/task').send({
      name: "",
      priority: "alta",
      description: "123"
    }).set('auth', token);
    ...
    const res_create_task3 = await request.post('/task').send({
  ...
  it('update task', async () => {
    const res_update_task = await request.put('/task/' + taskId).send({
